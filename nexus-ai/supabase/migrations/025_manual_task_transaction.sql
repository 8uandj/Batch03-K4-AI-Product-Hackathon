-- Atomic manual/ad-hoc task creation with audit and force-assign protection.
...
begin
  ...
  select allow_member_task_creation into member_task_creation
  from public.projects where id = target_project_id;
  if caller_role <> 'pm' and coalesce(member_task_creation, false) is not true then
    raise exception 'Member task creation is disabled for this project' using errcode = '42501';
  end if;
  ...
  if dependency_task_id is not null and not exists (select 1 from public.tasks where id = dependency_task_id and project_id = target_project_id) then
    raise exception 'Dependency task must belong to the same project' using errcode = '23514';
  end if;
  if source_task_id is not null and not exists (select 1 from public.tasks where id = source_task_id and project_id = target_project_id and status = 'done') then
    raise exception 'Source task must be a completed task in the same project' using errcode = '23514';
  end if;
  ...
  insert into public.assignment_decisions (
    ...
    target_project_id, created_task.id, auth.uid(),
  ...
  if caller_role = 'pm' and decision_risk in ('high', 'critical') and override_reason is not null and mitigation is not null then
    insert into public.deadline_notifications (
      ...
      '/project/' || target_project_id::text || '/board?task=' || created_task.id::text
    );
  end if;
