-- Make UUID joining idempotent and keep only one active request per user/email.
-- This complements 011_project_uuid_join_request.sql.
...
begin
  if caller_id is null then
    raise exception 'Bạn cần đăng nhập để tham gia project.';
  end if;
  ...
  if exists (
    ...
    where project_id = target_project_id and user_id = caller_id
  ) then
    return target_project_id;
  end if;

  if caller_email is null then
    raise exception 'Tài khoản chưa có email để tạo yêu cầu.';
  end if;

  -- Reuse an active invite/request instead of creating duplicates on retries.
  if not exists (
