...
import { createMockDashboardAnalytics } from "./mock-data.ts";
...
test("mock data tạo đủ thống kê và ít nhất một red flag", () => {
  ...
  assert.equal(result.stats.todo, 2);
  assert.equal(result.stats.doing, 2);
  assert.equal(result.stats.done, 2);
  assert.equal(result.stats.completionPercentage, 33);
  assert.equal(result.redFlags.length, 1);
});
